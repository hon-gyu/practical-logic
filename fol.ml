...
START_INTERACTIVE;;
{%fml|forall x y. exists z. x < z /\ y < z|};;

{%fml|~(forall x. P(x)) <=> exists y. ~P(y)|};;
END_INTERACTIVE;;
...
START_INTERACTIVE;;
holds bool_interp undefined {%fml|forall x. (x = 0) \/ (x = 1)|};;

holds (mod_interp 2) undefined {%fml|forall x. (x = 0) \/ (x = 1)|};;

holds (mod_interp 3) undefined {%fml|forall x. (x = 0) \/ (x = 1)|};;

let fm = {%fml|forall x. ~(x = 0) ==> exists y. x * y = 1|};;

filter (fun n -> holds (mod_interp n) undefined fm) (1--45);;

holds (mod_interp 3) undefined {%fml|(forall x. x = 0) ==> 1 = 0|};;
holds (mod_interp 3) undefined {%fml|forall x. x = 0 ==> 1 = 0|};;
END_INTERACTIVE;;
...
START_INTERACTIVE;;
subst ("y" |=> Var "x") {%fml|forall x. x = y|};;

subst ("y" |=> Var "x") {%fml|forall x x'. x = y ==> x = x'|};;
END_INTERACTIVE;;
