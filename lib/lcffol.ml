...
let%expect_test "eg: in the text" =
  let p58 = lcffol
   {%fol|forall x. exists v. exists w. forall y. forall z.
  ...
  [%expect {|
    ...
    |-
    forall x.
      exists v w. forall y z. P(x) /\ Q(y) ==> (P(v) \/ R(w)) /\ (R(z) ==> Q(v))Proving
    <<(forall x. x <= x) /\
      (forall x y z. x <= y /\ y <= z ==> x <= z) /\
      (forall x y. f(x) <= y <=> x <= g(y)) ==>
...
let%expect_test "eg: More exhaustive set of tests not in the main text" =
  ...
  let gilmore_9 = lcffol
   {%fol|forall x. exists y. forall z.
          ...
          ((forall u. exists v. F(x,u,v) /\ G(y,u) /\ ~H(x,y))
           ==> ~(forall u. exists v. F(x,u,v) /\ G(z,u) /\ ~H(x,z))
           ==> (forall u. exists v. F(y,u,v) /\ G(y,u) /\ ~H(y,x)) /\
               (forall u. exists v. F(z,u,v) /\ G(y,u) /\ ~H(z,y)))|} in
  print_thm
    (gilmore_9);
  let davis_putnam_example = lcffol
  ...
  [%expect {|
    ...
    |- (p \/ q ==> p \/ r) ==> p \/ (q ==> r)|- p \/ ~p|- p \/ ~(~(~p))|-
                                                                       ...
                                                                       p|-
                                                                        ...
                                                                        ~q \/
                                                                        ~q)
    |- (q ==> r) /\ (r ==> p /\ q) /\ (p ==> q /\ r) ==> (p <=> q)|- p <=> p
    |- ((p <=> q) <=> r) <=> p <=> q <=> r|- p \/ q /\ r <=> (p \/ q) /\ (p \/ r)
    |- (p <=> q) <=> (q \/ ~p) /\ (~q \/ p)|- p ==> q <=> ~p \/ q|-
                                                                 (p ==> q) \/
                                                                 (q ==> p)
    |- p /\ (q ==> r) ==> s <=> (~p \/ q \/ s) /\ (~p \/ ~r \/ s)Proving
    <<p ==> q <=> ~q ==> ~p>>
    Searching with depth limit 0
    |- p ==> q <=> ~q ==> ~pProving <<~(~p) <=> p>>
    Searching with depth limit 0
    |- ~(~p) <=> pProving <<~(p ==> q) ==> q ==> p>>
    Searching with depth limit 0
    |- ~(p ==> q) ==> q ==> pProving <<~p ==> q <=> ~q ==> p>>
    Searching with depth limit 0
    |- ~p ==> q <=> ~q ==> pProving <<(p \/ q ==> p \/ r) ==> p \/ (q ==> r)>>
    Searching with depth limit 0
    |- (p \/ q ==> p \/ r) ==> p \/ (q ==> r)Proving <<p \/ ~p>>
    Searching with depth limit 0
    |- p \/ ~pProving <<p \/ ~(~(~p))>>
    Searching with depth limit 0
    |- p \/ ~(~(~p))Proving <<((p ==> q) ==> p) ==> p>>
    Searching with depth limit 0
    |- ((p ==> q) ==> p) ==> pProving <<(p \/ q) /\ (~p \/ q) /\ (p \/ ~q) ==>
                                        ~(~q \/ ~q)>>
    Searching with depth limit 0
    |- (p \/ q) /\ (~p \/ q) /\ (p \/ ~q) ==> ~(~q \/ ~q)Proving <<(q ==> r) /\
                                                                   (r ==> p /\ q) /\
                                                                   (p ==> q /\ r) ==>
                                                                   (p <=> q)>>
    Searching with depth limit 0
    |- (q ==> r) /\ (r ==> p /\ q) /\ (p ==> q /\ r) ==> (p <=> q)Proving
    <<p <=> p>>
    Searching with depth limit 0
    |- p <=> pProving <<((p <=> q) <=> r) <=> p <=> q <=> r>>
    Searching with depth limit 0
    |- ((p <=> q) <=> r) <=> p <=> q <=> rProving <<p \/ q /\ r <=>
                                                    (p \/ q) /\ (p \/ r)>>
    Searching with depth limit 0
    |- p \/ q /\ r <=> (p \/ q) /\ (p \/ r)Proving <<(p <=> q) <=>
                                                     (q \/ ~p) /\ (~q \/ p)>>
    Searching with depth limit 0
    |- (p <=> q) <=> (q \/ ~p) /\ (~q \/ p)Proving <<p ==> q <=> ~p \/ q>>
    Searching with depth limit 0
    |- p ==> q <=> ~p \/ qProving <<(p ==> q) \/ (q ==> p)>>
    Searching with depth limit 0
    |- (p ==> q) \/ (q ==> p)Proving <<p /\ (q ==> r) ==> s <=>
                                       (~p \/ q \/ s) /\ (~p \/ ~r \/ s)>>
    Searching with depth limit 0
    |- p /\ (q ==> r) ==> s <=> (~p \/ q \/ s) /\ (~p \/ ~r \/ s)Proving
    <<exists y. forall x. P(y) ==> P(x)>>
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2
    |- exists y. forall x. P(y) ==> P(x)Proving <<exists x.
                                                    forall y z.
                                                      (P(y) ==> Q(z)) ==>
                                                      P(x) ==> Q(x)>>
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2
    |- exists x. forall y z. (P(y) ==> Q(z)) ==> P(x) ==> Q(x)Proving <<(
                                                                        forall x y.
                                                                        exists z.
                                                                        forall w.
                                                                        P(
    ...
    (exists x y. P(x) /\ Q(y)) ==> (exists z. R(z))Proving <<(exists x.
                                                                P ==> Q(x)) /\
                                                             (exists x.
                                                                Q(x) ==> P) ==>
                                                             (exists x.
    ...
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2
    |-
    (exists x. P ==> Q(x)) /\ (exists x. Q(x) ==> P) ==> (exists x. P <=> Q(x))Proving
    <<(forall x. P <=> Q(x)) ==> (P <=> (forall x. Q(x)))>>
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2
    |- (forall x. P <=> Q(x)) ==> (P <=> (forall x. Q(x)))Proving <<(forall x.
                                                                       P \/ Q(x)) <=>
                                                                    P \/
                                                                    (forall x.
                                                                       Q(x))>>
    Searching with depth limit 0Searching with depth limit 1
    |- (forall x. P \/ Q(x)) <=> P \/ (forall x. Q(x))Proving <<~(exists x.
                                                                    U(x) /\ Q(x)) /\
                                                                (forall x.
                                                                   P(x) ==>
                                                                   Q(x) \/ R(x)) /\
    ...
    (exists x. P(x) /\ R(x))Proving <<(exists x. P(x)) /\
                                      (forall x. U(x) ==> ~G(x) /\ R(x)) /\
                                      (forall x. P(x) ==> G(x) /\ U(x)) /\
                                      ((forall x. P(x) ==> Q(x)) \/
                                       (exists x. Q(x) /\ P(x))) ==>
    ...
    (forall x. P(x) ==> G(x) /\ U(x)) /\
    ((forall x. P(x) ==> Q(x)) \/ (exists x. Q(x) /\ P(x))) ==>
    (exists x. Q(x) /\ P(x))Proving <<((exists x. P(x)) <=> (exists x. Q(x))) /\
                                      (forall x y.
                                         P(x) /\ Q(y) ==> (R(x) <=> U(y))) ==>
                                      ((forall x. P(x) ==> R(x)) <=>
                                       (forall x. Q(x) ==> U(x)))>>
    ...
    ((exists x. P(x)) <=> (exists x. Q(x))) /\
    (forall x y. P(x) /\ Q(y) ==> (R(x) <=> U(y))) ==>
    ((forall x. P(x) ==> R(x)) <=> (forall x. Q(x) ==> U(x)))Proving <<(exists x.
                                                                        P(
                                                                        x) /\
                                                                        ~Q(x)) /\
                                                                       (forall x.
    ...
    (forall x. U(x) ==> ~R(x)) ==> (forall x. U(x) ==> ~V(x))Proving <<(forall x.
                                                                        P(
                                                                        x) ==>
                                                                        (
                                                                        forall x.
    ...
    ((forall x. Q(x) \/ R(x)) ==> (exists x. Q(x) /\ R(x))) /\
    ((exists x. R(x)) ==> (forall x. L(x) ==> M(x))) ==>
    (forall x. P(x) /\ L(x) ==> M(x))Proving <<(exists x. P(x)) /\
                                               (exists x. G(x)) ==>
                                               ((forall x. P(x) ==> H(x)) /\
                                                (forall x. G(x) ==> J(x)) <=>
                                                (forall x y.
    ...
    ((forall x. P(x) ==> H(x)) /\ (forall x. G(x) ==> J(x)) <=>
     (forall x y. P(x) /\ G(y) ==> H(x) /\ J(y)))Proving <<(forall x.
                                                              P(x) \/ G(x) ==>
                                                              ~H(x)) /\
                                                           (forall x.
                                                              (G(x) ==> ~U(x)) ==>
    ...
    (forall x. (G(x) ==> ~U(x)) ==> P(x) /\ H(x)) ==> (forall x. U(x))Proving
    ...
    ~(exists x. P(x) /\ (G(x) \/ H(x))) /\
    (exists x. Q(x) /\ P(x)) /\ (forall x. ~H(x) ==> J(x)) ==>
    (exists x. Q(x) /\ J(x))Proving <<(forall x. P(x) /\ (G(x) \/ H(x)) ==> Q(x)) /\
                                      (forall x. Q(x) /\ H(x) ==> J(x)) /\
                                      (forall x. R(x) ==> H(x)) ==>
    ...
    (forall x. P(x) /\ R(x) ==> J(x))Proving <<(forall x.
                                                  P(a) /\ (P(x) ==> P(b)) ==>
                                                  P(c)) <=>
    ...
    (forall x. P(a) ==> P(x) \/ P(c)) /\ (P(a) ==> P(b) ==> P(c))Proving
    <<exists x y. P(x,y) ==> (forall x y. P(x,y))>>
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2
    |- exists x y. P(x,y) ==> (forall x y. P(x,y))Proving <<(forall x.
                                                               exists y. P(x,y)) /\
                                                            (forall x.
                                                               exists y. G(x,y)) /\
                                                            (forall x y.
    ...
    (forall x. exists y. H(x,y))Proving <<(forall z.
                                             exists w.
                                               forall x.
                                                 exists y.
                                                   (P(x,z) ==> P(y,w)) /\
    ...
    (forall x z. ~P(x,z) ==> (exists y. Q(y,z))) /\
    ((exists x y. Q(x,y)) ==> (forall x. R(x,x))) ==>
    (forall x. exists y. R(x,y))Proving <<(forall x.
                                             P(a) /\
                                             (P(x) ==> (exists y. P(y) /\ R(x,y))) ==>
                                             (exists z w.
                                                P(z) /\ R(x,w) /\ R(w,z))) <=>
                                          (forall x.
                                             (~P(a) \/
    ...
    (forall x.
       (~P(a) \/ P(x) \/ (exists z w. P(z) /\ R(x,w) /\ R(w,z))) /\
       (~P(a) \/
        ~(exists y. P(y) /\ R(x,y)) \/ (exists z w. P(z) /\ R(x,w) /\ R(w,z))))Proving
    <<~(exists x. forall y. P(y,x) <=> ~P(y,y))>>
    Searching with depth limit 0Searching with depth limit 1
    |- ~(exists x. forall y. P(y,x) <=> ~P(y,y))Proving <<(exists y.
                                                             forall x.
                                                               P(x,y) <=> P(x,x)) ==>
                                                          ~(forall x.
                                                              exists y.
    ...
    ~(forall x. exists y. forall z. P(z,y) <=> ~P(z,x))Proving <<(forall z.
                                                                    exists y.
                                                                      forall x.
                                                                        P(
                                                                        x,y) <=>
    ...
    ~(exists z. forall x. P(x,z))Proving <<~(exists y.
                                               forall x.
                                                 P(x,y) <=>
                                                 ~(exists z. P(x,z) /\ P(z,x)))>>
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2Searching with depth limit 3Searching with depth limit 4
    |- ~(exists y. forall x. P(x,y) <=> ~(exists z. P(x,z) /\ P(z,x)))Proving
    <<(forall x.
    ...
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2Searching with depth limit 3
    |-
    (forall x. P(x) ==> (exists y. G(y) /\ H(x,y)) /\ (exists y. G(y) /\ ~H(x,y))) /\
    (exists x. J(x) /\ (forall y. G(y) ==> H(x,y))) ==> (exists x. J(x) /\ ~P(x))Proving
    <<lives(agatha) /\
      lives(butler) /\
      lives(charles) /\
    ...
    (forall x. hates(agatha,x) ==> hates(butler,x)) /\
    (forall x. ~hates(x,agatha) \/ ~hates(x,butler) \/ ~hates(x,charles)) ==>
    killed(agatha,agatha) /\ ~killed(butler,agatha) /\ ~killed(charles,agatha)Proving
    <<P(f(a,b),f(b,c)) /\
    ...
    P(f(a,b),f(b,c)) /\
    P(f(b,c),f(a,c)) /\ (forall x y z. P(x,y) /\ P(y,z) ==> P(x,z)) ==>
    P(f(a,b),f(a,c))Proving <<forall P Q R x.
                                exists v w.
                                  forall y z.
                                    P(x) /\ Q(y) ==>
                                    (P(v) \/ R(w)) /\ (R(z) ==> Q(v))>>
    ...
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2Searching with depth limit 3
    |- (forall x. P(x) <=> ~P(f(x))) ==> (exists x. P(x) /\ ~P(f(x)))Proving
    <<forall x.
        P(x,f(x)) <=> (exists y. (forall z. P(z,y) ==> P(z,f(x))) /\ P(x,y))>>
    Searching with depth limit 0Searching with depth limit 1
    |-
    forall x.
      P(x,f(x)) <=> (exists y. (forall z. P(z,y) ==> P(z,f(x))) /\ P(x,y))Proving
    <<exists x.
        forall y z.
          ((F(y,z) ==> G(y) ==> H(x)) ==> F(x,x)) /\
    ...
    exists x.
      forall y z.
        ((F(y,z) ==> G(y) ==> H(x)) ==> F(x,x)) /\
        ((F(z,x) ==> G(x)) ==> H(z)) /\ F(x,y) ==> F(z,z)Proving <<exists x y.
                                                                     forall z.
                                                                       (F(
                                                                        x,y) ==>
                                                                        F(
    ...
    exists x y.
      forall z.
        (F(x,y) ==> F(y,z) /\ F(z,z)) /\ (F(x,y) /\ G(x,y) ==> G(x,z) /\ G(z,z))Proving
    <<(forall x. exists y. F(x,y) \/ F(y,x)) /\ (forall x y. F(y,x) ==> F(y,y)) ==>
      (exists z. F(z,z))>>
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2Searching with depth limit 3Searching with depth limit 4
    |-
    (forall x. exists y. F(x,y) \/ F(y,x)) /\ (forall x y. F(y,x) ==> F(y,y)) ==>
    (exists z. F(z,z))Proving <<forall x.
                                  exists y.
                                    (exists u.
                                       forall v. F(u,x) ==> G(v,u) /\ G(u,x)) ==>
                                    (exists u.
                                       forall v. F(u,y) ==> G(v,u) /\ G(u,y)) \/
                                    (forall u v.
                                       exists w. G(v,u) \/ H(w,y,u) ==> G(u,w))>>
    Searching with depth limit 0Searching with depth limit 1Searching with depth limit 2Searching with depth limit 3
    |-
    forall x.
      exists y.
        (exists u. forall v. F(u,x) ==> G(v,u) /\ G(u,x)) ==>
        (exists u. forall v. F(u,y) ==> G(v,u) /\ G(u,y)) \/
        (forall u v. exists w. G(v,u) \/ H(w,y,u) ==> G(u,w))Proving <<(forall x.
                                                                        K(
                                                                        x) ==>
                                                                        (
                                                                        exists y.
    ...
    (exists z. K(z) /\ (forall u. L(u) ==> F(z,u))) ==>
    (exists v w. K(v) /\ L(w) /\ G(v,w))Proving <<exists x.
                                                    forall y z.
                                                      ((F(y,z) ==>
                                                        G(y) ==>
                                                        (forall u.
      ...
      exists y.
        forall z.
          ...
          ((forall u. exists v. F(x,u,v) /\ G(y,u) /\ ~H(x,y)) ==>
           ~(forall u. exists v. F(x,u,v) /\ G(z,u) /\ ~H(x,z)) ==>
           (forall u. exists v. F(y,u,v) /\ G(y,u) /\ ~H(y,x)) /\
           (forall u. exists v. F(z,u,v) /\ G(y,u) /\ ~H(z,y)))Proving <<
                                                                       exists x y.
                                                                        forall z.
                                                                        (
                                                                        F(
    ...
    (forall x y z. x <= y /\ y <= z ==> x <= z) /\
    (forall x y. f(x) <= y <=> x <= g(y)) ==>
    (forall x y. x <= y ==> f(x) <= f(y))Proving <<(forall x. x <= x) /\
                                                   (forall x y z.
                                                      x <= y /\ y <= z ==> x <= z) /\
                                                   (forall x y.
                                                      f(x) <= y <=> x <= g(y)) ==>
