(* Goedel's theorem and relatives.  Kept in its own library because it builds
   the Robinson arithmetic derivations eagerly at module initialisation, which
...
let robinson_thm =
  ...
  [note("eq_refl",{%fol|forall x. x = x|}) using [axiom_eqrefl (Var "x")];
   ...
   note("suc_cong",{%fol|forall a b. a = b ==> S(a) = S(b)|})
      using [axiom_funcong "S" [Var "a"] [Var "b"]];
   note("add_cong",
        {%fol|forall a b c d. a = b /\ c = d ==> a + c = b + d|})
   ...
   assume ["suc_inj",{%fol|forall m n. S(m) = S(n) ==> m = n|};
           "num_nz",{%fol|forall n. ~(n = 0) <=> exists m. n = S(m)|};
           "add_0",{%fol|forall n. 0 + n = n|};
           "add_suc",{%fol|forall m n. S(m) + n = S(m + n)|};
           "mul_0",{%fol|forall n. 0 * n = 0|};
           "mul_suc",{%fol|forall m n. S(m) * n = n + m * n|};
           "le_def",{%fol|forall m n. m <= n <=> exists d. m + d = n|};
           "lt_def",{%fol|forall m n. m < n <=> S(m) <= n|}];
   note("not_suc_0",{%fol|forall n. ~(S(n) = 0)|}) by ["num_nz"; "eq_refl"];
   so conclude {%fol|forall n. S(n) = 0 ==> false|} at once;
   so conclude {%fol|forall n. 0 = S(n) ==> false|} by ["eq_sym"];
   note("num_cases",{%fol|forall n. (n = 0) \/ exists m. n = S(m)|})
         by ["num_nz"];
   note("suc_inj_eq",{%fol|forall m n. S(m) = S(n) <=> m = n|})
     by ["suc_inj"; "suc_cong"];
   so conclude
     {%fol|forall m n. (m = n ==> false) ==> (S(m) = S(n) ==> false)|}
     at once;
   conclude {%fol|forall m n. (exists d. m + d = n) ==> m <= n|}
     by ["le_def"];
   conclude {%fol|forall m n. S(m) <= n ==> m < n|} by ["lt_def"];
   conclude {%fol|forall m n. (forall d. d <= n ==> d = m ==> false)
                          ==> m <= n ==> false|}
     by ["eq_refl"; "le_cong"];
   conclude {%fol|forall m n. (forall d. d < n ==> d = m ==> false)
                          ==> m < n ==> false|}
     by ["eq_refl"; "lt_cong"];
   have {%fol|0 <= 0|} by ["le_def"; "add_0"];
   so have {%fol|forall x. x = 0 ==> x <= 0|}
     by ["le_cong"; "eq_refl"; "eq_sym"];
   so conclude {%fol|forall n. n <= 0 \/ (exists m. S(m) = n)|}
     by ["num_nz"; "eq_sym"];
   note("add_eq_0",{%fol|forall m n. m + n = 0 ==> m = 0 /\ n = 0|}) proof
    [fix "m"; fix "n";
     assume ["A",{%fol|m + n = 0|}];
     cases {%fol|m = 0 \/ exists p. m = S(p)|} by ["num_cases"];
       so conclude {%fol|m = 0|} at once;
       so have {%fol|m + n = 0 + n|} by ["add_cong"; "eq_refl"];
       so our thesis by ["A"; "add_0"; "eq_sym"; "eq_trans"];
     qed;
       so consider ("p",{%fol|m = S(p)|}) at once;
       so have {%fol|m + n = S(p) + n|} by ["add_cong"; "eq_refl"];
       so have {%fol|m + n = S(p + n)|} by ["eq_trans"; "add_suc"];
       so have {%fol|S(p + n) = 0|} by ["A"; "eq_sym"; "eq_trans"];
       so our thesis by ["not_suc_0"];
     qed];
   so conclude {%fol|forall n. n <= 0 ==> n = 0|} by ["le_def"];
   have {%fol|forall m n. S(m) <= S(n) ==> m <= n|} proof
    [fix "m"; fix "n";
     assume ["lesuc",{%fol|S(m) <= S(n)|}];
     so consider("d",{%fol|S(m) + d = S(n)|}) by ["le_def"];
     so have {%fol|S(m + d) = S(n)|} by ["add_suc"; "eq_sym"; "eq_trans"];
     so have {%fol|m + d = n|} by ["suc_inj"];
     so conclude {%fol|m <= n|} by ["le_def"];
     qed];
   so conclude {%fol|forall m n. S(m) <= S(n) ==> m <= n|} at once;
   so conclude {%fol|forall m n. m < S(n) ==> m <= n|} by ["lt_def"];
   fix "n";
   assume ["hyp",{%fol|n < 0|}];
   so have {%fol|S(n) <= 0|} by ["lt_def"];
   so consider("d",{%fol|S(n) + d = 0|}) by ["le_def"];
   so have {%fol|S(n + d) = 0|} by ["add_suc"; "eq_trans"; "eq_sym"];
   so our thesis by ["not_suc_0"];
   qed];;
