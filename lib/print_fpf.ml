...
(* Lazy so that merely linking the library does not read this file: the
   executable's working directory is not generally the repo root. *)
let func_data = lazy (read_whole_file "lib/print_fpf.ml");;

let words = lazy (Str.split (Str.regexp "[^a-zA-Z]+") (Lazy.force func_data));;
...
let test1() =
  let p2 x =
    ...
    Format.close_box() in
  Format.open_box(1);
  list_iter_sep p2 Format.print_space (Lazy.force words);
  Format.close_box();;
