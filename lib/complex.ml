...
let%expect_test "eg: Sanity check" =
  print_fol_formula
    (polyatom ["w"; "x"; "y"; "z"]
      {%fol|((w + x)^4 + (w + y)^4 + (w + z)^4 +
         (x + y)^4 + (x + z)^4 + (y + z)^4 +
         (w - x)^4 + (w - y)^4 + (w - z)^4 +
         (x - y)^4 + (x - z)^4 + (y - z)^4) / 6 =
        (w^2 + x^2 + y^2 + z^2)^2|});
...
let%expect_test "eg: Examples" =
  print_fol_formula
    (complex_qelim
     {%fol|forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 1 = 0|});
  print_fol_formula
    (complex_qelim
     {%fol|forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + c = 0|});
  print_fol_formula
    (complex_qelim
     {%fol|forall c.
  ...
  *********)
  ...
  complex_qelim_all
   {%fol|(2 * u2 * x2 + 2 * u3 * x1 - u3^2 - u2^2 = 0) /\
    (2 * u1 * x2 - u1^2 = 0) /\
