...
START_INTERACTIVE;;
tab {%fml|forall a. ~(P(a) /\ (forall y z. Q(y) \/ R(z)) /\ ~P(a))|};;

tab {%fml|forall a. ~(P(a) /\ ~P(a) /\ (forall y z. Q(y) \/ R(z)))|};;
...
let davis_putnam_example = meson
 {%fml|exists x. exists y. forall z.
        (F(x,y) ==> (F(y,z) /\ F(z,z))) /\
...
let steamroller = meson
 {%fml|((forall x. P1(x) ==> P0(x)) /\ (exists x. P1(x))) /\
...
let p18 = time meson
 {%fml|exists y. forall x. P(y) ==> P(x)|};;

let p19 = time meson
 {%fml|exists x. forall y z. (P(y) ==> Q(z)) ==> P(x) ==> Q(x)|};;
...
let p21 = time meson
 {%fml|(exists x. P ==> Q(x)) /\ (exists x. Q(x) ==> P)
   ==> (exists x. P <=> Q(x))|};;

let p22 = time meson
 {%fml|(forall x. P <=> Q(x)) ==> (P <=> (forall x. Q(x)))|};;

let p23 = time meson
 {%fml|(forall x. P \/ Q(x)) <=> P \/ (forall x. Q(x))|};;

let p24 = time meson
 {%fml|~(exists x. U(x) /\ Q(x)) /\
...
let p26 = time meson
 {%fml|((exists x. P(x)) <=> (exists x. Q(x))) /\
   (forall x y. P(x) /\ Q(y) ==> (R(x) <=> U(y))) ==>
   ((forall x. P(x) ==> R(x)) <=> (forall x. Q(x) ==> U(x)))|};;

let p27 = time meson
 {%fml|(exists x. P(x) /\ ~Q(x)) /\
   (forall x. P(x) ==> R(x)) /\
...
let p30 = time meson
 {%fml|(forall x. P(x) \/ G(x) ==> ~H(x)) /\ (forall x. (G(x) ==> ~U(x)) ==>
     P(x) /\ H(x)) ==>
   (forall x. U(x))|};;

let p31 = time meson
 {%fml|~(exists x. P(x) /\ (G(x) \/ H(x))) /\ (exists x. Q(x) /\ P(x)) /\
   (forall x. ~H(x) ==> J(x)) ==>
   (exists x. Q(x) /\ J(x))|};;
...
let p35 = time meson
 {%fml|exists x y. P(x,y) ==> (forall x y. P(x,y))|};;
...
let p39 = time meson
 {%fml|~(exists x. forall y. P(y,x) <=> ~P(y,y))|};;

let p40 = time meson
 {%fml|(exists y. forall x. P(x,y) <=> P(x,x))
  ==> ~(forall x. exists y. forall z. P(z,y) <=> ~P(z,x))|};;

let p41 = time meson
 {%fml|(forall z. exists y. forall x. P(x,y) <=> P(x,z) /\ ~P(x,x))
  ==> ~(exists z. forall x. P(x,z))|};;

let p42 = time meson
 {%fml|~(exists y. forall x. P(x,y) <=> ~(exists z. P(x,z) /\ P(z,x)))|};;

let p43 = time meson
 {%fml|(forall x y. Q(x,y) <=> forall z. P(z,x) <=> P(z,y))
   ==> forall x y. Q(x,y) <=> Q(y,x)|};;

let p44 = time meson
 {%fml|(forall x. P(x) ==> (exists y. G(y) /\ H(x,y)) /\
   (exists y. G(y) /\ ~H(x,y))) /\
...
let p46 = time meson
 {%fml|(forall x. P(x) /\ (forall y. P(y) /\ H(y,x) ==> G(y)) ==> G(x)) /\
   ((exists x. P(x) /\ ~G(x)) ==>
    (exists x. P(x) /\ ~G(x) /\
...
let p58 = time meson
 {%fml|forall P Q R. forall x. exists v. exists w. forall y. forall z.
    ((P(x) /\ Q(y)) ==> ((P(v) \/ R(w))  /\ (R(z) ==> Q(v))))|};;

let p59 = time meson
 {%fml|(forall x. P(x) <=> ~P(f(x))) ==> (exists x. P(x) /\ ~P(f(x)))|};;

let p60 = time meson
 {%fml|forall x. P(x,f(x)) <=>
...
let gilmore_5 = time meson
 {%fml|(forall x. exists y. F(x,y) \/ F(y,x)) /\
   (forall x y. F(y,x) ==> F(y,y))
   ==> exists z. F(z,z)|};;

let gilmore_6 = time meson
 {%fml|forall x. exists y.
        (exists u. forall v. F(u,x) ==> G(v,u) /\ G(u,x))
        ==> (exists u. forall v. F(u,y) ==> G(v,u) /\ G(u,y)) \/
            (forall u v. exists w. G(v,u) \/ H(w,y,u) ==> G(u,w))|};;

let gilmore_7 = time meson
 {%fml|(forall x. K(x) ==> exists y. L(y) /\ (F(x,y) ==> G(x,y))) /\
...
let gilmore_9 = time meson
 {%fml|forall x. exists y. forall z.
