...
START_INTERACTIVE;;
integer_qelim {%fml|forall x y. ~(2 * x + 1 = 2 * y)|};;

integer_qelim {%fml|forall x. exists y. 2 * y <= x /\ x < 2 * (y + 1)|};;

integer_qelim {%fml|exists x y. 4 * x - 6 * y = 1|};;

integer_qelim {%fml|forall x. ~divides(2,x) /\ divides(3,x-1) <=>
                          divides(12,x-1) \/ divides(12,x-7)|};;

integer_qelim {%fml|forall x. b < x ==> a <= x|};;
END_INTERACTIVE;;
...
START_INTERACTIVE;;
natural_qelim {%fml|forall d. exists x y. 3 * x + 5 * y = d|};;
integer_qelim {%fml|forall d. exists x y. 3 * x + 5 * y = d|};;
natural_qelim {%fml|forall d. d >= 8 ==> exists x y. 3 * x + 5 * y = d|};;
natural_qelim {%fml|forall d. exists x y. 3 * x - 5 * y = d|};;
...
integer_qelim {%fml|exists x y. x > 0 /\ y >= 0 /\ 3 * x - 5 * y = 1|};;

integer_qelim {%fml|exists x y z. 4 * x - 6 * y = 1|};;

integer_qelim {%fml|forall x. a < 3 * x ==> b < 3 * x|};;

time integer_qelim {%fml|forall x y. x <= y ==> 2 * x + 1 < 2 * y|};;

time integer_qelim {%fml|(exists d. y = 65 * d) ==> (exists d. y = 5 * d)|};;

time integer_qelim
  {%fml|forall y. (exists d. y = 65 * d) ==> (exists d. y = 5 * d)|};;

time integer_qelim {%fml|forall x y. ~(2 * x + 1 = 2 * y)|};;

time integer_qelim
  {%fml|forall x y z. (2 * x + 1 = 2 * y) ==> x + y + z > 129|};;

time integer_qelim {%fml|forall x. a < x ==> b < x|};;
...
time integer_qelim {%fml|forall a b. exists x. a < 20 * x /\ 20 * x < b|};;

time integer_qelim {%fml|exists x. a < 20 * x /\ 20 * x < b|};;

time integer_qelim {%fml|forall b. exists x. a < 20 * x /\ 20 * x < b|};;

time integer_qelim
  {%fml|forall a. exists b. a < 4 * b + 3 * a \/ (~(a < b) /\ a > b + 1)|};;

time integer_qelim
  {%fml|exists y. forall x. x + 5 * y > 1 /\ 13 * x - y > 1 /\ x + 2 < 0|};;

(* ------------------------------------------------------------------------- *)
(* More of my own.                                                           *)
(* ------------------------------------------------------------------------- *)

time integer_qelim {%fml|forall x y. x >= 0 /\ y >= 0
                  ==> 12 * x - 8 * y < 0 \/ 12 * x - 8 * y > 2|};;

time integer_qelim {%fml|exists x y. 5 * x + 3 * y = 1|};;

time integer_qelim {%fml|exists x y. 5 * x + 10 * y = 1|};;

time integer_qelim {%fml|exists x y. x >= 0 /\ y >= 0 /\ 5 * x - 6 * y = 1|};;


time integer_qelim {%fml|exists w x y z. 2 * w + 3 * x + 4 * y + 5 * z = 1|};;

time integer_qelim {%fml|exists x y. x >= 0 /\ y >= 0 /\ 5 * x - 3 * y = 1|};;

time integer_qelim {%fml|exists x y. x >= 0 /\ y >= 0 /\ 3 * x - 5 * y = 1|};;

time integer_qelim {%fml|exists x y. x >= 0 /\ y >= 0 /\ 6 * x - 3 * y = 1|};;

time integer_qelim
  {%fml|forall x y. ~(x = 0) ==> 5 * y < 6 * x \/ 5 * y > 6 * x|};;

time integer_qelim
  {%fml|forall x y. ~divides(5,x) /\ ~divides(6,y) ==> ~(6 * x = 5 * y)|};;

time integer_qelim {%fml|forall x y. ~divides(5,x) ==> ~(6 * x = 5 * y)|};;

time integer_qelim {%fml|forall x y. ~(6 * x = 5 * y)|};;

time integer_qelim {%fml|forall x y. 6 * x = 5 * y ==> exists d. y = 3 * d|};;

time integer_qelim {%fml|6 * x = 5 * y ==> exists d. y = 3 * d|};;
...
time integer_qelim
  {%fml|forall z. z > 7 ==> exists x y. x >= 0 /\ y >= 0 /\ 3 * x + 5 * y = z|};;

time integer_qelim
  {%fml|forall z. z > 2 ==> exists x y. x >= 0 /\ y >= 0 /\ 3 * x + 5 * y = z|};;
...
time integer_qelim
  {%fml|forall x. ~divides(2,x) /\ divides(3,x-1) <=>
              divides(12,x-1) \/ divides(12,x-7)|};;

time integer_qelim
  {%fml|forall x. ~(exists m. x = 2 * m) /\ (exists m. x = 3 * m + 1) <=>
...
(integer_qelim ** generalize)
  {%fml|a + 2 = b /\ v_3 = b - a + 1 /\ v_2 = b - 2 /\ v_1 = 3 ==> false|};;
...
integer_qelim
  {%fml|exists a b. ~(a = 1) /\ ((2 * b = a) \/ (2 * b = 3 * a + 1)) /\ (a = b)|};;

integer_qelim
 {%fml|exists a b. a > 1 /\ b > 1 /\
               ((2 * b = a) \/ (2 * b = 3 * a + 1)) /\
               (a = b)|};;
...
integer_qelim
  {%fml|forall x. x >= 8 ==> exists u v. u >= 0 /\ v >= 0 /\ x = 3 * u + 5 * v|};;
