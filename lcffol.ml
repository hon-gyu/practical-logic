...
lcfrefute {%fml|p(1) /\ ~q(1) /\ (forall x. p(x) ==> q(x))|} 1 simpcont;;

lcfrefute {%fml|(exists x. ~p(x)) /\ (forall x. p(x))|} 1 simpcont;;
...
START_INTERACTIVE;;
let p58 = lcffol
 {%fml|forall x. exists v. exists w. forall y. forall z.
