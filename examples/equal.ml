(* Interactive examples from equal.ml, lifted out of the source.  Not compiled: these are meant to be
...
let fm =
 {%fol|(forall x y z. eqA (x * (y * z),(x * y) * z)) /\
...
let fm =
 {%fol|(forall x y z. eq1(x * (y * z),(x * y) * z)) /\
