...
let fm =
 {%fml|(forall x y z. x * (y * z) = (x * y) * z) /\ p * q * p = p
...
meson
 {%fml|(forall x. (forall w. R'(x,w) <=> R'(x,w))) /\
...
meson
 {%fml|(forall x y. R(x,y) <=> (forall z. R'(x,z) <=> R'(y,z))) /\
   (forall x. R'(x,x))
   ==> forall x y. ~R'(x,y) ==> ~R(x,y)|};;

meson
 {%fml|(forall x y. R(x,y) <=> (forall z. R'(y,z) ==> R'(x,z))) /\
   (forall x. R'(x,x))
   ==> forall x y. ~R'(x,y) ==> ~R(x,y)|};;

meson
 {%fml|(forall x y. R(x,y) <=> R'(x,y) /\ R'(y,x))
   ==> forall x y. ~R'(x,y) ==> ~R(x,y)|};;
