...
START_INTERACTIVE;;
complex_qelim
 {%fml|forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 1 = 0|};;

complex_qelim
 {%fml|forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + c = 0|};;
...
time complex_qelim {%fml|exists x. x + 2 = 3|};;

time complex_qelim {%fml|exists x. x^2 + a = 3|};;

time complex_qelim {%fml|exists x. x^2 + x + 1 = 0|};;

time complex_qelim {%fml|exists x. x^2 + x + 1 = 0 /\ x^3 + x^2 + 1 = 0|};;

time complex_qelim {%fml|exists x. x^2 + 1 = 0 /\ x^4 + x^3 + x^2 + x = 0|};;

time complex_qelim {%fml|forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 1 = 0|};;

time complex_qelim {%fml|forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 2 = 0|};;

time complex_qelim {%fml|exists a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 /\ ~(x^4 + 2 = 0)|};;

time complex_qelim {%fml|exists x. a^2 = 2 /\ x^2 + a * x + 1 = 0 /\ ~(x^4 + 2 = 0)|};;

time complex_qelim {%fml|forall x. x^2 + a * x + 1 = 0 ==> x^4 + 2 = 0|};;

time complex_qelim {%fml|forall a. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 2 = 0|};;

time complex_qelim {%fml|exists a b c x y.
        a * x^2 + b * x + c = 0 /\
        a * y^2 + b * y + c = 0 /\
        ~(x = y) /\
        ~(a * x * y = c)|};;
...
time complex_qelim
 {%fml|forall x y. x^2 = 2 /\ y^2 = 3
         ==> (x * y)^2 = 6|};;

time complex_qelim
 {%fml|forall x a. (a^2 = 2) /\ (x^2 + a * x + 1 = 0)
         ==> (x^4 + 1 = 0)|};;

time complex_qelim
 {%fml|forall a x. (a^2 = 2) /\ (x^2 + a * x + 1 = 0)
         ==> (x^4 + 1 = 0)|};;

time complex_qelim
 {%fml|~(exists a x y. (a^2 = 2) /\
             (x^2 + a * x + 1 = 0) /\
             (y * (x^4 + 1) + 1 = 0))|};;

time complex_qelim {%fml|forall x. exists y. x^2 = y^3|};;

time complex_qelim
 {%fml|forall x y z a b. (a + b) * (x - y + z) - (a - b) * (x + y + z) =
               2 * (b * x + b * z - a * y)|};;

time complex_qelim
 {%fml|forall a b. ~(a = b) ==> exists x y. (y * x^2 = a) /\ (y * x^2 + x = b)|};;

time complex_qelim
 {%fml|forall a b c x y. (a * x^2 + b * x + c = 0) /\
               (a * y^2 + b * y + c = 0) /\
               ~(x = y)
               ==> (a * x * y = c) /\ (a * (x + y) + b = 0)|};;
...
time complex_qelim
 {%fml|forall c x y. (a * x^2 + b * x + c = 0) /\
              (a * y^2 + b * y + c = 0) /\
              (forall z. (a * z^2 + b * z + c = 0)
                         ==> (z = x) \/ (z = y))
              ==> (a * x * y = c) /\ (a * (x + y) + b = 0)|};;
...
time complex_qelim
 {%fml|~(forall x1 y1 x2 y2 x3 y3 x0 y0 x0' y0'.
        (x1 - x0)^2 + (y1 - y0)^2 =
        (x2 - x0)^2 + (y2 - y0)^2 /\
        (x2 - x0)^2 + (y2 - y0)^2 =
        (x3 - x0)^2 + (y3 - y0)^2 /\
...
time complex_qelim {%fml|forall y_1 y_2 y_3 y_4.
       (y_1 = 2 * y_3) /\
       (y_2 = 2 * y_4) /\
       (y_1 * y_3 = y_2 * y_4)
       ==> (y_1^2 = y_2^2)|};;
...
complex_qelim_all
 {%fml|(2 * u2 * x2 + 2 * u3 * x1 - u3^2 - u2^2 = 0) /\
  (2 * u1 * x2 - u1^2 = 0) /\
...
time complex_qelim {%fml|forall y.
         a * x^2 + b * x + c = 0 /\
         a * y^2 + b * y + c = 0 /\
         ~(x = y)
         ==> a * x * y = c /\ a * (x + y) + b = 0|};;
...
time complex_qelim
  {%fml|forall x y. x^2 + y^2 = 1 ==> (2 * y^2 - 1)^2 + (2 * x * y)^2 = 1|};;

(* ------------------------------------------------------------------------- *)
(* The examples from my thesis.                                              *)
(* ------------------------------------------------------------------------- *)

time complex_qelim {%fml|forall s c. s^2 + c^2 = 1
      ==> 2 * s - (2 * s * c * c - s^3) = 3 * s^3|};;

time complex_qelim {%fml|forall u v.
  -((((9 * u^8) * v) * v - (u * u^9)) * 128) -
     (((7 * u^6) * v) * v - (u * u^7)) * 144 -
     (((5 * u^4) * v) * v - (u * u^5)) * 168 -
     (((3 * u^2) * v) * v - (u * u^3)) * 210 -
...
time complex_qelim {%fml|exists z. x * z^87 + y * z^44 + 1 = 0|};;

time complex_qelim {%fml|forall u. exists v. x * (u + v^2)^2 + y * (u + v^2) + z = 0|};;

(* ------------------------------------------------------------------------- *)
(* Actually prove simple equivalences.                                       *)
(* ------------------------------------------------------------------------- *)

time complex_qelim {%fml|forall x y. (exists z. x * z^87 + y * z^44 + 1 = 0)
                  <=> ~(x = 0) \/ ~(y = 0)|};;

time complex_qelim {%fml|forall x y z. (forall u. exists v.
...
time complex_qelim {%fml|exists w x y z. (a * w + b * y = 1) /\
                      (a * x + b * z = 0) /\
                      (c * w + d * y = 0) /\
                      (c * x + d * z = 1)|};;

time complex_qelim {%fml|forall a b c d.
...
time complex_qelim
  {%fml|forall x y. (a * x + b * y = u * x - v * y) /\
                (c * x + d * y = u * y + v * x)
                ==> (a = d)|};;
...
complex_qelim
  {%fml|forall x1 y1 x2 y2. exists a b.
